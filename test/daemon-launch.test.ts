...
describe('launchWhenSteamReady — retrying', () => {
  it('asks again when the app does not come up', async () => {
    const deps = makeDeps({ steamReadyAfter: 0, appAfterLaunches: 3 });
    expect(await launchWhenSteamReady(deps, { ...FAST, maxAttempts: 3 })).toBe('launched');
    expect(deps.launches()).toBe(3);
  });

  it('gives up after the agreed number of attempts', async () => {
    const deps = makeDeps({ steamReadyAfter: 0, appAfterLaunches: null });
    expect(await launchWhenSteamReady(deps, { ...FAST, maxAttempts: 2 })).toBe('gave-up');
