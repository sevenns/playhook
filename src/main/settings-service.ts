  ...
  init(): void {
    ...
    ipcMain.on(IPC.settingsSetOnlyGlobalAmbient, (_event, on: boolean) => {
      void this.deps.settings
        ...
        .catch((cause: unknown) => log.error('[settings] failed to persist only-global-ambient:', cause));
    });
    ...
    ipcMain.on(IPC.settingsSetSteamGridDbKey, (_event, key: string) => {
      void this.deps.settings
        ...
        .catch((cause: unknown) => log.error('[settings] failed to persist the SteamGridDB key:', cause));
    });
