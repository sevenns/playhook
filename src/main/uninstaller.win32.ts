...
// registry). Split out of the controller: the pure parts (the silent flags per installer family, the
// CommandLineToArgvW parser, the in-dir search) are unit-tested on their own — see
// test/uninstaller.test.ts. The directory sweep every uninstall path ends in is OS-neutral and lives in
// remove-with-retry.ts.
