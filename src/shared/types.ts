// Shared contract between main, preload and renderer: the types both sides read, plus the IPC channel
// table and `MAX_HERO_IMAGES`, which ARE runtime values — preload imports the table. Everything else is
// erased; the renderer imports it via `import type`. What only main reads (the manifest domain model,
...
// ── What main pushes to the launcher: app state and the per-channel payloads ──────────────────
 ...
 * travels over IPC to the renderers; the raw mode only lives in the settings form.
 */
...
export interface AppSettings {
  ...
  /** UI language. Default 'system' → resolved from the OS locale in main. */
...
// ── Config editing: drives, validation and the file picker ────────────────────────────────────
